...
def main():
    print("Starting robust standard labor PDF extraction (v14)...")
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for p in range(30, total_pages):
            ...
            for table_idx, table in enumerate(tables):
                ...
                if process_groups:
                    for start, end, name in process_groups:
                        for idx in range(start, end + 1):
                            if idx < len(row1):
                                cell_clean = row1[idx].replace("\n", "").replace(" ", "")
                                if any(lt in cell_clean for lt in LABOR_TYPES):
                        ...
                        if matched_labor:
                            ...
                            for start, end, name in process_groups:
                                if start <= col_idx <= end:
                                    proc_name = name
                                    break
                            columns_map[col_idx] = (clean_value_string(proc_name), matched_labor)
                else:
                ...
                # Factor cols helper for job_col_idx block
                factor_cols = []
                if job_col_idx != -1:
                    for idx, cell in enumerate(row0):
                        if idx not in [name_idx, spec_idx, unit_idx, job_col_idx] and cell.strip():
                            factor_cols.append((idx, clean_value_string(cell)))
                ...
                for r_idx in range(data_start_row, len(clean_table_rows)):
                    ...
                    # Analyze if this is a wrapped single line or multiple actual lines of factor values.
                    val_lines_count = 1
                    if columns_map:
                        for col_idx in columns_map.keys():
                            if col_idx < len(row):
                                val_lines_count = max(val_lines_count, len(row[col_idx].split("\n")))
                    elif job_col_idx != -1:
                        for col_idx, proc_name in factor_cols:
                            if col_idx < len(row):
                                val_lines_count = max(val_lines_count, len(row[col_idx].split("\n")))
                                
                    if val_lines_count == 1:
                        names_split = [" ".join(names_split)]
                        specs_split = [" ".join(specs_split)]
                        units_split = [" ".join(units_split)]
                        max_lines = 1
                    else:
