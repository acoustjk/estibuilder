...
print("Patching app.js with multi-labor support...")
...
# 2. Replace recommendLaborBasis function
recommend_pattern = r"function recommendLaborBasis\(itemName, itemSpec\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}"
recommend_js = """function recommendLaborBasis(itemName, itemSpec) {
...
content = re.sub(recommend_pattern, recommend_js, content)
print("Updated recommendLaborBasis function.")

# 3. Replace autoMapAllLaborBasis function
auto_map_pattern = r"function autoMapAllLaborBasis\(\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}"
auto_map_js = """function autoMapAllLaborBasis() {
    let count = 0;
...
content = re.sub(auto_map_pattern, auto_map_js, content)
...
modal_block_pattern = r"function openLaborRefModal\(divId, itemId\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}\s*// 모달 내 실시간 노무 단가 프리뷰 업데이트\s*function updateLaborModalPreview\(laborFactor, laborType\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}\s*// 모달 내 품셈 검색\s*function searchModalLaborBasis\(query\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}\s*// 품셈 적용 완료 및 갱신\s*function applyLaborRef\(code, laborType, laborFactor\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}"
new_modal_block_js = """function openLaborRefModal(divId, itemId) {
...
content = re.sub(modal_block_pattern, new_modal_block_js, content)
print("Updated modal functions.")

# 5. Replace openAddPriceItemModal and confirmAddPriceItem
add_price_block_pattern = r"function openAddPriceItemModal\(\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}\s*// 2\. 단가조사 직접 추가 처리\s*function confirmAddPriceItem\(\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}"
new_add_price_js = """function openAddPriceItemModal() {
...
content = re.sub(add_price_block_pattern, new_add_price_js, content)
print("Updated add/confirm price items.")

# 6. Replace renderLaborBasisTable function
render_table_pattern = r"function renderLaborBasisTable\(\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}"
new_render_table_js = """function renderLaborBasisTable() {
...
content = re.sub(render_table_pattern, new_render_table_js, content)
print("Updated renderLaborBasisTable function.")

# 7. Replace calculateEstimates function
calc_pattern = r"function calculateEstimates\(\) \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}"
new_calc_js = """function calculateEstimates() {
    const wages = WAGE_RATES[state.wageStandard];
...
content = re.sub(calc_pattern, new_calc_js, content)
print("Updated calculateEstimates function.")

# 8. Update activeDiv.items.push(newItem) inside catalog click handler (around 1945)
catalog_add_pattern = r"const newItem = \{\s*id: \"item-\" \+ Date\.now\(\) \+ Math\.random\(\)\.toString\(36\)\.substr\(2, 5\),\s*masterId: dbItem\.id,\s*name: dbItem\.name,\s*spec: dbItem\.spec,\s*unit: dbItem\.unit,\s*qty: 1,\s*materialPrice: dbItem\.materialPrice,\s*laborType: dbItem\.laborType,\s*laborFactor: dbItem\.laborFactor,\s*laborScenario: \"new\",\s*laborMultiplier: 1\.0,\s*laborRef: dbItem\.laborRef \|\| \"\",\s*laborRemark: \"신설\"\s*\};"
new_catalog_add_js = """const newItem = {
            id: "item-" + Date.now() + Math.random().toString(36).substr(2, 5),
            masterId: dbItem.id,
            name: dbItem.name,
            spec: dbItem.spec,
...
content = re.sub(catalog_add_pattern, new_catalog_add_js, content)
print("Updated catalog newItem structure.")

# 9. Replace exportToExcel pre-calculations and sheet exports
excel_precalc_pattern = r"// Pre-calculate Excel row indices for 1-to-1 linkage between '내역서' and '노임근거'\s*let boqRow = 3;\s*let laborRow = 2;\s*state\.divisions\.forEach\(div => \{\s*boqRow\+\+;\s*div\.items\.forEach\(item => \{\s*item\.excelRowIndex = boqRow;\s*boqRow\+\+;\s*if \(item\.laborType && item\.laborFactor > 0\) \{\s*item\.laborExcelRowIndex = laborRow;\s*laborRow\+\+;\s*\} else \{\s*item\.laborExcelRowIndex = null;\s*\}\s*\}\);\s*boqRow\+\+;\s*boqRow\+\+;\s*boqRow\+\+;\s*\}\);"
new_excel_precalc_js = """// Pre-calculate Excel row indices for 1-to-many linkage between '내역서' and '노임근거'
        let boqRow = 3;
        let laborRow = 2;
...
content = re.sub(excel_precalc_pattern, new_excel_precalc_js, content)
print("Updated excel row pre-calculations.")

# 10. Replace shLabor sheet generation block in exportToExcel
sh_labor_pattern = r"const shLabor = workbook\.addWorksheet\(\"노임근거\"\);(?:[^\n]*\n)*?\s*shLabor\.getColumn\(11\)\.numFmt = \"₩#,##0\";\s*shLabor\.getColumn\(12\)\.numFmt = \"₩#,##0\";"
new_sh_labor_js = """const shLabor = workbook.addWorksheet("노임근거");
...
content = re.sub(sh_labor_pattern, new_sh_labor_js, content)
print("Updated shLabor sheet generation.")

# 11. Replace shBOQ item iteration block in exportToExcel
sh_boq_item_pattern = r"// Write items\s*div\.items\.forEach\(\(item, idx\) => \{(?:[^{}]*|\{(?:[^{}]*|\{[^{}]*\})*\})*\}\);"
new_sh_boq_item_js = """// Write items
...
content = re.sub(sh_boq_item_pattern, new_sh_boq_item_js, content)
