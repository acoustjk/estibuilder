...
print("Starting standard labor PDF extraction (v4)...")
...
for page_idx in range(30, 300):
    ...
    for i, cm in enumerate(clean_matches):
        ...
        for lt in LABOR_TYPES:
            # Match with optional spaces between characters
        ...
        for m_idx, m in enumerate(matches):
            ...
            # Remove labor types using regex (which handles spaces!)
            for lt in LABOR_TYPES:
                pattern = "".join([re.escape(c) + r"\s*" for c in lt])
            ...
            # Clean any trailing garbage or leftover unit numbers from names
            full_item_name = re.sub(r'\s*\d+\s*(?:m|개|대|세트|조|본|km|Port|소|개소|열)\s*$', '', full_item_name)
            full_item_name = full_item_name.strip()
                ...
                if col_idx < len(factors):
                    ...
                    if f_val != "-":
                        try:
                            ...
                            # CRITICAL FILTER: daily labor factor must be between 0.0001 and 8.0
                            if 0.0001 <= factor_float <= 8.0:
                                # Generate rich keywords
                                keywords = [full_item_name[:20]]
                                if "UTP" in full_item_name or "꼬임" in full_item_name or "cable" in full_item_name.lower():
                                    keywords.extend(["utp", "케이블", "cable", "꼬임"])
                                if "광섬유" in full_item_name or "광케이블" in full_item_name:
                                    keywords.extend(["광케이블", "광섬유", "광", "fiber", "optical"])
                                if "배관" in full_item_name or "합성수지" in full_item_name or "pvc" in full_item_name.lower() or "관로" in full_item_name:
...
print(f"Extraction complete! Successfully parsed {len(parsed_items)} clean standard labor items.")
