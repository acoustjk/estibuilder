...
LABOR_TYPES = ["통신내선공", "통신설비공", "통신외선공", "통신케이블공", "특별인부", "보통인부", "광케이블설치사", 
                "H/W시험사", "S/W시험사", "통신관련기사", "통신관련산업기사", "무선안테나공", "특별인부", "보통인부", "임전기공"]
...
def main():
    print("Starting robust standard labor PDF extraction (v8)...")
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for p in range(30, total_pages):
            ...
            for table_idx, table in enumerate(tables):
                ...
                for r_idx, row in enumerate(clean_table_rows):
                    for col_idx, cell in enumerate(row):
                        ...
                        if cell_clean in ["〃", "〃〃", "”", "””", "〃〃〃"]:
                            ...
                            # Replace ditto symbol with parent value suffix or just clean it
                ...
                # Determine name, spec, unit indices
                name_idx = 0
                spec_idx = 1 if len(row0) > 1 else -1
                unit_idx = 2 if len(row0) > 2 else -1
                ...
                for r_idx in range(data_start_row, len(clean_table_rows)):
                    ...
                    # Split cells by newline to support sub-rows
                    ...
                    if columns_map:
                        ...
                        for sub_idx in range(max_lines):
                            ...
                            sub_unit = units_split[sub_idx].strip() or default_unit
                            ...
                            for proc_name, labors in process_labors.items():
                                ...
                                parsed_items.append({
                                    ...
                                    "category": category,
                                    "page": p + 1,
                                    "keywords": list(set([item_name.lower(), sub_spec.lower(), table_title.lower()]))
                                })
                    elif job_col_idx != -1:
                        # Mapped by Job/Labor Type column in row
                        # e.g., 종류 | 단위 | 직종 | 적상 | 적하
                        # 콘크리트전봇대 | 기 | 보통인부 | 0.38 | 0.25
                        job_cell = row[job_col_idx]
                        jobs_split = job_cell.split("\n")
                        jobs_split += [jobs_split[-1]] * (max_lines - len(jobs_split))
                        
                        # Find numeric columns for factors (e.g. 적상, 적하)
                        factor_cols = []
                        for idx, cell in enumerate(row0):
                        ...
                        for sub_idx in range(max_lines):
                            ...
                            sub_spec = specs_split[sub_idx].strip()
                            sub_unit = units_split[sub_idx].strip() or default_unit
                            sub_job = jobs_split[sub_idx].strip()
                            ...
                            # Emit separate items for each factor column process
                            for col_idx, proc_name in factor_cols:
                                val_str = col_factors[col_idx][sub_idx].strip()
                                try:
                                    val = float(val_str)
